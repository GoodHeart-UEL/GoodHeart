...
import 'package:good_heart_new/Components/alert_dialog.dart';
import 'package:good_heart_new/Components/display_files.dart';
import 'package:good_heart_new/Components/show_evaluation.dart';
import 'package:good_heart_new/Services/communication_with_server.dart';
import 'package:good_heart_new/Screens/connection_page.dart';
import 'package:good_heart_new/Services/json_to_list.dart';
import '../main.dart';
  ...
  void startSocketListenInEvaluationPage() {
    socket!.listener.listen((List<int> bytes) async {
      ...
      var receivedFromServer =
          CommunicationWithServer.fromJson(jsonDecode(stringBytesReceived));
      ...
      switch (receivedFromServer.OpCode) {
        case 610:
          ...
          setState(() {
            ...
            logger
                .d("[$LoggerClass] Ecg list of files from json: $_listOfFiles");
          });
          ...
          if (globals.chosenFileName != null) {
            logger
                .d("[$LoggerClass] chosenFileName: ${globals.chosenFileName}");
            globals.idMsgValue += 1;
            var fileChoice = CommunicationWithServer(
                IdMsg: globals.idMsgValue,
                OpCode: 100,
                ECGFile: globals.chosenFileName);
          ...
          children: <Widget>[
            ...
            Card(
              child: ListTile(
                ...
                subtitle: Text(
                  'Capture new ECG by placing sensors all over patients chest.',
            ...
            Card(
              child: ListTile(
                ...
                subtitle: Text(
                  'Find a previously generated file with ECG info.',
                  style: TextStyle(height: 1.3),
                ),
                isThreeLine: true,
                onTap: () async {
                  try {
                    globals.idMsgValue += 1;
                    var sendToServer = CommunicationWithServer(
                        IdMsg: globals.idMsgValue, OpCode: 600);
                    var sendToServerJson = sendToServer.toJson();
                    socket!.client!.write(sendToServerJson);
                    logger.d(
                        "[$LoggerClass] Sending ecg file request to server: $sendToServerJson");
                  } catch (_) {
                    await alertDialog(
                      context,
