...
import 'package:good_heart_new/main.dart';
import 'package:animated_text_kit/animated_text_kit.dart';
import 'package:good_heart_new/globals.dart' as globals;
...
class _ConnectionPage extends State<ConnectionPage> {
  ...
  final TextEditingController _textEditingControllerConnectionTest =
      TextEditingController();
  ...
  void startSocketListenInConnectionPage() {
    socket!.listener.listen((List<int> bytes) async {
      // AQUI acho que não tem o await
      var receivedFromServer = CommunicationWithServer.fromJson(
          jsonDecode(new String.fromCharCodes(bytes).trim()));
  ...
  Future<void> showAlertServerAnswer(BuildContext context) async {
    return await showDialog(
        ...
        builder: (context) {
          return AlertDialog(
              content:
                  _checkingConnectionText(_textEditingControllerConnectionTest),
              actions: <Widget>[
                TextButton(
                  onPressed: () {
                    Navigator.of(context).pop();
  ...
  Future<void> showAlertErrorSocket(BuildContext context) async {
    return await showDialog(
        context: context,
        builder: (context) {
          return AlertDialog(content: Text("Socket ERROR."), actions: <Widget>[
            TextButton(
              onPressed: () {
                Navigator.of(context).pop();
              },
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: ListView(children: <Widget>[
          Card(
            //AQUI não sei se esse align faz sentido dentro de um listview, já que ele não faz nada
            child: Image.asset(
              ...
              onTap: () async {
                try {
                  var sendToServer = CommunicationWithServer(
                      IdMsg: globals.idMsgValue, OpCode: 700);
            ...
            child: ListTile(
              ...
              onTap: () async {
                try {
                  ...
                  var sendToServer = CommunicationWithServer(
                      IdMsg: globals.idMsgValue, OpCode: 500);
                  socket!.client!.write(sendToServer.toJson());
                } catch (_) {
                  _textEditingControllerConnectionTest.text = "Not connected";
                  await showAlertServerAnswer(context);
  ...
  return SizedBox(
    ...
    child: DefaultTextStyle(
      ...
      child: AnimatedTextKit(
        ...
        animatedTexts: [
          TyperAnimatedText('Checking...', speed: Duration(milliseconds: 100)),
          ColorizeAnimatedText("\"" + _text.text + "\".",
              textStyle: colorizeTextStyle,
              colors: [
                Color.fromRGBO(0, 0, 0, 1),
